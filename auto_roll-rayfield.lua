-- ================================================
-- Auto Roll v12 — Random Anime Tower Defense
...
-- ================================================
-- Rarity detection — reads the bottom unit tray
-- After a roll, a new card appears at the bottom showing the rolled unit.
-- The rarity label on that card is what we want.
-- We snapshot the tray BEFORE rolling and check what's NEW after.
-- ================================================

-- Find all rarity-coloured TextLabels in the bottom inventory bar
local function GetTrayRarities()
    local results = {}
    ...
    pcall(function()
        for _, obj in ipairs(PlayerGui:GetDescendants()) do
            ...
            if t:find("[^%a]") then continue end  -- letters only
            local tl = t:lower()
            if not RARITY_SET[tl] then continue end
            -- Must be in bottom 35% of screen (the unit tray)
            local pos = obj.AbsolutePosition
            if pos.Y < vp.Y * 0.6 then continue end
            results[obj] = tl
        end
    end)
    return results
...
local function HighestRarityInTray(tray)
...
local function DetectNewRarity(before)
    -- Check what appeared in the tray that wasn't there before
    local found, foundRank = nil, 0
    local after = GetTrayRarities()
    for obj, tl in pairs(after) do
        if not before[obj] then  -- new label
            local r = RARITY_SET[tl]
            if r and r.rank > foundRank then found = r.name; foundRank = r.rank end
        end
    end
    -- Fallback: just read highest in tray (covers case where tray re-uses same labels)
    if not found then
        found = HighestRarityInTray(after)
    end
    return found
end
...
local function DoRoll()
    ...
    -- Wait for animation + new card to appear
    task.wait(math.max(State.rollDelay * 0.7, 0.35))
