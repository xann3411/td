...
-- (odds boards, shop previews, unit stat cards, etc.)
...
local function IsBlacklisted(obj)
    -- Walk up the ancestor chain checking names
    local cur = obj
    for _ = 1, 6 do
...
local function DetectRarity()
    ...
    pcall(function()
        for _, obj in ipairs(PlayerGui:GetDescendants()) do
            ...
            -- The rarity label must be the ONLY text in it (exact match, no extra words)
            local t = obj.Text:match("^%s*(.-)%s*$")
            if not t or #t == 0 or #t > 20 then continue end  -- rarity names are short

            -- Must not contain numbers, $, %, "in", "rolls" — filters out odds text
            if t:find("%d") or t:find("%$") or t:find("%%") then continue end
            local tl = t:lower()
            if tl:find(" in ") or tl:find("roll") or tl:find("wave") or tl:find("dps") then continue end
