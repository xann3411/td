-- ╔══════════════════════════════════════════════════════════════╗
-- ║       BridgeNet2 Tower Defense Macro — GUI Edition v2        ║
-- ╚══════════════════════════════════════════════════════════════╝
...
-- ┌─────────────────────────────────────┐
-- │  UIScale — scales EVERYTHING inside │
-- └─────────────────────────────────────┘
local uiScaleVal = 0.75  -- start smaller for mobile

-- ── Main Frame (fixed pixel size; UIScale shrinks it) ───────────
local BASE_W = 560
local BASE_H = 400
local BASE_H_MIN = 36
...
-- THE key fix: UIScale on Main scales every child proportionally
local UIScaleObj = Instance.new("UIScale", Main)
UIScaleObj.Scale = uiScaleVal

-- Helper: get the actual on-screen size of Main
...
ScaleFill.Size = UDim2.new(0.5, 0, 1, 0)  -- 75% = halfway in 0.5-1.0 range
...
-- Invisible hit area for scale slider (tall for touch)
...
-- ── Floating Toggle Button (always visible when minimized) ───────
...
-- ── Helper: toggle row ───────────────────────────────────────────
-- Returns (setFn, getFn, row).  Pass an onToggle callback for side effects.
local function makeToggleRow(labelText, order, defaultOn, onToggle)
    local row = Instance.new("TextButton")  -- TextButton = whole row is tappable
    ...
    -- Whole row tap: flip state then call the callback with new state
...
local setAutoEquip, getAutoEquip, _autoRow = makeToggleRow("Auto Equip Macro Units", 6, true, nil)

local setRecord, getRecord, _recRow = makeToggleRow("Record Macro", 7, false, function(nowOn)
    if nowOn then startRecording() else stopRecording() end
end)

local setPlay, getPlay, _playRow = makeToggleRow("Play Macro", 8, false, function(nowOn)
    if nowOn then startPlayback() else stopPlayback() end
end)

local setLoop, getLoop, _loopRow = makeToggleRow("Loop Playback", 9, false, function(nowOn)
    loopPlay = nowOn
end)
...
-- Tall invisible hit area for slider
...
local function setMinimized(state)
    ...
    -- Tween the unscaled height; UIScale handles the visual shrink
    ...
    playThread = task.spawn(function()
        repeat
            ...
            if loopPlay and playing then
                flashStatus("Looping...", Color3.fromRGB(120, 180, 255))
                task.wait(0.5)
            end
...
print("[TD Macro GUI v2] Loaded! F5=Record  F6=Play  or tap the toggles.")
