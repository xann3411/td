...
-- Rarity detection
-- ================================================
-- Blacklist: GUI names that contain rarity text but are NOT roll results
local DETECTION_BLACKLIST = {
    "oddboard", "odds", "rate", "chance", "info", "preview",
    "stat", "shop", "index", "upgrade", "details", "tooltip",
    "billboard", "surfacegui", "description", "prob", "pity",
    "limited", "offer", "pack", "store", "bundle", "banner",
    "starter", "group", "reward", "daily", "event", "time",
}
local function IsBlacklisted(obj)
    ...
    for _ = 1, 8 do
        ...
        for _, bad in ipairs(DETECTION_BLACKLIST) do
...
local function IsNearCenter(obj)
    -- Roll result popups appear in the center area of the screen
    -- Reject anything in the far right/left edges (ads, banners, shop UI)
    local ok, pos = pcall(function() return obj.AbsolutePosition end)
    if not ok then return true end -- can't check, allow it
    local ok2, vp = pcall(function() return workspace.CurrentCamera.ViewportSize end)
    if not ok2 then return true end
    local screenW = vp.X
    -- Reject labels in the rightmost 30% or leftmost 20% of screen
    if pos.X > screenW * 0.75 then return false end
    if pos.X < screenW * 0.15 then return false end
    return true
end
...
local function DetectRarity()
    ...
    pcall(function()
        for _, obj in ipairs(PlayerGui:GetDescendants()) do
            ...
            if IsBlacklisted(obj) then continue end
            if not IsNearCenter(obj) then continue end

            local t = obj.Text:match("^%s*(.-)%s*$")
            if not t or #t == 0 or #t > 20 then continue end

            -- Must be plain rarity text — no numbers, $, %, special keywords
            if t:find("%d") or t:find("%$") or t:find("%%") then continue end
            local tl = t:lower()
            if tl:find(" in ") or tl:find("roll") or tl:find("wave")
                or tl:find("dps") or tl:find("time") or tl:find("pack")
                or tl:find("tier") or tl:find("gold") or tl:find("lv")
                or tl:find("level") or tl:find("base") then continue end

            for _, rar in ipairs(RARITIES) do
                if tl == rar.name:lower() and rar.rank > foundRank then
                    found = rar.name
                    foundRank = rar.rank
                end
            end
